...
function* syncCollection({ collection }: any, user: UserState) {
  ...
  if (
    ...
    if (localCollection) {
      const r = yield call(requests.setCollection, localCollection || {}, user);
      console.log({ r });
    }
  }
...
function* syncSquadrons({ squadrons, removed }: any, user: UserState) {
  ...
  if (toSend.length > 0) {
    ...
    for (var i = 0; i < toSend.length; i++) {
      const r = yield call(requests.setSquadron, toSend[i], user);
      console.log({ r });
    }
  }
...
function* syncBlueprints(
  ...
  for (var i = 0; i < toSend.length; i++) {
    const r = yield call(requests.setBlueprint, toSend[i], user);
    console.log({ r });
  }
...
function* syncTournaments(
  ...
  for (var i = 0; i < toSend.length; i++) {
    const r = yield call(requests.setTournament, toSend[i], user);
    console.warn(`SEND ERRORS: ${r.errors}`);
  }
...
function* doSync(): any {
  ...
  try {
    ...
    const result = yield call(requests.syncRequest, user);
    console.log({ result });
